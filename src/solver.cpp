...
void Solver::syncLeft() {
    ...
    if(cell) {
      ...
      if(cell) {
        for(int gasIndex=0;gasIndex<P->gas.size();gasIndex++) {
          MPI::COMM_WORLD.Recv(cell->m_half[gasIndex], P->impulse->value.size(), MPI::DOUBLE, P->mpi_rank - 1, 50+gasIndex);
          MPI::COMM_WORLD.Recv(cell->m_value[gasIndex], P->impulse->value.size(), MPI::DOUBLE, P->mpi_rank - 1, 60+gasIndex);
        }
      }
    }
  }
  ...
  for(int y=0;y<Ny;y++) {
    ...
    if(cell1) {
      for(int gasIndex=0;gasIndex<P->gas.size();gasIndex++) {
        MPI::COMM_WORLD.Ssend(cell1->m_half[gasIndex], P->impulse->value.size(), MPI::DOUBLE, P->mpi_rank - 1, 50+gasIndex);
        MPI::COMM_WORLD.Ssend(cell1->m_value[gasIndex], P->impulse->value.size(), MPI::DOUBLE, P->mpi_rank - 1, 60+gasIndex);
      }
      Cell* cell2 = GetCell(1, y);
      if(cell2) {
        for(int gasIndex=0;gasIndex<P->gas.size();gasIndex++) {
          MPI::COMM_WORLD.Ssend(cell2->m_half[gasIndex], P->impulse->value.size(), MPI::DOUBLE, P->mpi_rank - 1, 50+gasIndex);
          MPI::COMM_WORLD.Ssend(cell2->m_value[gasIndex], P->impulse->value.size(), MPI::DOUBLE, P->mpi_rank - 1, 60+gasIndex);
        }
      }
    }
  }
...
void Solver::syncRight() {
  for(int y=0;y<Ny;y++) {
    ...
    if(cell) {
      for(int gasIndex=0;gasIndex<P->gas.size();gasIndex++) {
        MPI::COMM_WORLD.Ssend(cell->m_half[gasIndex], P->impulse->value.size(), MPI::DOUBLE, P->mpi_rank + 1, 50+gasIndex);
        MPI::COMM_WORLD.Ssend(cell->m_value[gasIndex], P->impulse->value.size(), MPI::DOUBLE, P->mpi_rank + 1, 60+gasIndex);
      }
    }
  }
  ...
  for(int y=0;y<Ny;y++) {
    ...
    if(cell1) {
      ...
      if(cell1) {
        for(int gasIndex=0;gasIndex<P->gas.size();gasIndex++) {
          MPI::COMM_WORLD.Recv(cell1->m_half[gasIndex], P->impulse->value.size(), MPI::DOUBLE, P->mpi_rank + 1, 50+gasIndex);
          MPI::COMM_WORLD.Recv(cell1->m_value[gasIndex], P->impulse->value.size(), MPI::DOUBLE, P->mpi_rank + 1, 60+gasIndex);
        }
        Cell* cell2 = cell1->m_next[Cell::X];
        if(cell2) {
          for(int gasIndex=0;gasIndex<P->gas.size();gasIndex++) {
            MPI::COMM_WORLD.Recv(cell2->m_half[gasIndex], P->impulse->value.size(), MPI::DOUBLE, P->mpi_rank + 1, 50+gasIndex);
            MPI::COMM_WORLD.Recv(cell2->m_value[gasIndex], P->impulse->value.size(), MPI::DOUBLE, P->mpi_rank + 1, 60+gasIndex);
          }
        }
      }
    }
